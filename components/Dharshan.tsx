...
export default function Dharshan(props: JSX.IntrinsicElements["group"]) {
  const group = useRef<THREE.Group | null>(null) // ✅ null-safe
  const mouse = useRef(new THREE.Vector2())
  const { progress, total } = useProgress()
  const [isIntroAnimationDone, setIsIntroAnimationDone] = useState(false)

  const { scene } = useGLTF("/model/Dharshan-transformed.glb") as unknown as GLTFResult
  const clone = useMemo(() => SkeletonUtils.clone(scene), [scene])
  const { nodes, materials } = useGraph(clone) as unknown as GLTFResult

  // GSAP intro animation
  useEffect(() => {
    if (total === 20 && progress === 100 && group.current) {
...
useGLTF.preload("/model/Dharshan-transformed.glb")
